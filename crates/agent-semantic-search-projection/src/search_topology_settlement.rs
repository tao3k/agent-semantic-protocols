...
use crate::search_topology_settlement_support::{
    digest_json, is_blake3_digest, require_text_eq, required_array, required_object,
    required_object_field, required_text, required_u64, validate_projection,
};

#[path = "search_topology_settlement_render.rs"]
mod render;
...
impl SearchTopologySettlement {
    ...
    pub fn from_workspace_result(
        ...
        for (rank, item) in evidence.iter().enumerate() {
            ...
            if parsed.owner_path().map_err(|cause| {
                ...
                || selector
                    .split_once("#item/")
                    .map_or(item_identity != "owner-root", |(_, identity)| {
            ...
            let Some(parsed) = parsed.parser_item() else {
                continue;
            };
            let node_id = format!(
                "grounded-{}",
        ...
        for node in library.search_nodes_by_id(&selected_ids) {
            ...
            } else if let Some(owner_locator) = node.get("ownerLocator").and_then(Value::as_str)
                && let Some(rank) = rank_by_selector.get(owner_locator).copied()
            {
                projected.insert(
                    "projection".to_owned(),
                    serde_json::json!({
                        "rank": rank,
                        "depth": 0,
                        "hit": hit_by_selector[owner_locator].clone(),
                    }),
                );
            }
    ...
    pub fn admit(packet: Value) -> Result<Self, SearchTopologySettlementError> {
        ...
        for node in nodes {
            ...
            let projection = node.get("projection");
            let queryable_endpoint = node.get("selector").and_then(Value::as_str).or_else(|| {
                projection
                    .is_some()
                    .then(|| node.get("ownerLocator").and_then(Value::as_str))
                    .flatten()
            });
            if let Some(endpoint) = queryable_endpoint
                && !selectors.insert(endpoint)
            {
                return invalid(
                    "duplicate-selector",
                    format!("queryable endpoint {endpoint} is owned by more than one settled node"),
                );
            }
            ...
            "fixed-point" => {
                ...
                if result_state != expected_result {
                    return invalid(
                        ...
                        "result state does not match queryable GQL endpoints",
                    );
                }
            }
            "budget-exhausted" => {
    ...
    /// Number of canonical item selectors or ranked owner roots exposed on the GQL nodes.
    pub fn queryable_endpoint_count(&self) -> usize {
        self.packet["nodes"].as_array().map_or(0, |nodes| {
            nodes
                .iter()
                .filter(|node| {
                    node.get("selector").is_some()
                        || (node.get("projection").is_some() && node.get("ownerLocator").is_some())
                })
                .count()
        })
    }
