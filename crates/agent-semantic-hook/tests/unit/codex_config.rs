...
fn claude_settings_merge_preserves_unmanaged_hooks_and_replaces_managed_hooks() {
    let existing = r##"{
      "hooks": {
        "PreToolUse": [
          ...
          {"matcher": "*", "hooks": [{"type": "command", "command": "# Generated by asp install hook --client claude.\nasp-hook pre-tool --client claude --old"}]}
        ],
        "PermissionRequest": [
          {"matcher": "*", "hooks": [{"type": "command", "command": "# Generated by asp install hook --client claude.\nasp hook permission-request --client claude --old"}]}
        ]
      }
    }"##;
    let merged =
    ...
    assert!(merged.contains("# Generated by asp install hook --client claude."));
    assert!(merged.contains("hook_bin="));
    assert!(merged.contains("pre-tool --client claude"));
