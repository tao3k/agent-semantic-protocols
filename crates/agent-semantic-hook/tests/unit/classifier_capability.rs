...
fn config_selected_explorer_role_admits_search_across_codex_surfaces() {
    let payloads = [
        serde_json::json!({
            ...
            "tool_input": {"cmd": "asp rust search pipe 'HookDecision' --workspace . --view seeds"}
        }),
        serde_json::json!({
            "agentRole": "asp-explorer",
            "tool_name": "Bash",
            "tool_input": {"command": "env ASP_VIEW=seeds asp rust search pipe 'HookDecision' --workspace . --view seeds"}
        }),
    ];
