...
fn registered_external_providers_reference_valid_workspace_install_descriptors() {
    ...
    for (language_id, expected_reference, descriptor_path) in WORKSPACE_INSTALL_DESCRIPTORS {
        let registration =
        ...
        let descriptor_json = fs::read_to_string(
            Path::new(env!("CARGO_MANIFEST_DIR"))
                .join("../..")
                .join(descriptor_path),
        )
        .expect("read workspace install descriptor");
