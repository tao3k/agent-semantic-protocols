use agent_semantic_hook::{
    ...
    ResidentSandboxMode, classify_hook, render_platform_response,
};
use serde_json::json;
...
fn permission_context<'a>(
    ...
    HookSubagentPermissionContext::new(
        ...
        ManagedChildName::new("asp-explore").expect("managed child name"),
        ConfiguredCodexAgentName::new("asp_explorer").expect("configured Codex agent name"),
        ConfiguredResidentRole::new("asp_explorer").expect("configured resident role"),
        Some(CodexHookAgentId::new(agent_id).expect("Codex hook agent id")),
        Some(CodexHookAgentType::new(agent_type).expect("Codex hook agent type")),
        identity_proof,
        child_session_id
            .map(|value| ResidentChildSessionId::new(value).expect("resident child session id")),
        identity_status,
