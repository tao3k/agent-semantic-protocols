...
#[test]
fn pre_tool_allows_root_state_locate_command_without_facade_feedback() {
    let project_root = temp_project_root("asp-hook-root-state-locate-command");
    let runtime = runtime_for_project(&project_root);
    ...
    let decision = classify_hook(
        ...
        &json!({
            ...
            "transcript_path": "transcript-state.jsonl",
            "tool_name": "Bash",
            "tool_input": {
                "command": "asp state locate --json"
            }
        }),
    );
