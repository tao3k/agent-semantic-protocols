        ...
        &[
            ...
            Scenario {
                ...
                payload: serde_json::json!({
                    ...
                    "tool_input": {
                        "code": "const r = await tools.exec_command({cmd: \"asp search playbook '(search (producers (language rust)) (intersect (rg \\\"HookDecision\\\" \\\".\\\") (tantivy \\\"body:HookDecision\\\")))'\"});"
                    }
                }),
                expected_rule: Some("registered-asp-reasoning-search"),
                forbidden_rule: None,
...
fn registered_asp_query_projects_structured_projection_across_codex_surfaces() {
    let command = "asp query playbook '(query (producers (language rust)) (select (selectors \"rust://crates/example.rs#item/function/example\") (projection source)))'";
    run_scenarios(
        ...
        &[
            ...
            Scenario {
                ...
                payload: shell_surface(
                    "Bash",
                    "command",
                    "rtk --ultra-compact err asp query playbook '(query (producers (language rust)) (select (selectors \"rust://crates/example.rs#item/function/example\") (projection source)))'",
                ),
                expected_rule: Some("registered-asp-structured-projection"),
                forbidden_rule: None,
            },
            Scenario {
                ...
                payload: serde_json::json!({
                    "tool_name": "functions.exec",
                    "tool_input": {
                        "code": "const r = await tools.exec_command({cmd: \"asp query playbook '(query (producers (language rust)) (select (selectors \\\"rust://crates/example.rs#item/function/example\\\") (projection source)))'\"});"
                    }
                }),
                expected_rule: Some("registered-asp-structured-projection"),
                forbidden_rule: None,
