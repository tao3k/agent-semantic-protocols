...
fn user_prompt_search_first_context(locator_only: bool) -> &'static str {
    if locator_only {
        return "ASP Search playbook routing is active for this locator question. Compose one public `asp search playbook '<scheme-expression>'` request. Put code and document producers in `(producers (language ...) (documents ...))`. Choose exactly the primary leaf justified by the predicate: `topology (owners ...)` for file/path membership over the resident generation, `rg` for regex truth over source bytes, `tantivy` for ranked prose, `syntax` for structural discovery, and `native-syntax` only for a known canonical selector. There is no default engine pair; use `intersect` only for an explicit conjunction over independently complete sets, `chain` only for a typed scope transform, and `graph` only as the final barrier. Preserve native argv tokens as Scheme strings and keep Workspace scope at top level. Consume the returned closure state and copy Agent-chosen canonical root or item selectors unchanged into one `(query (producers ...) (select (selectors \"...\") (projection source)))` request through `asp query playbook '<scheme-expression>'`. Complete ASP evidence forbids a second `find`, `fd`, `sed`, or external `rg` discovery pass; only a typed ASP unavailable/failure terminal authorizes recovery. Query does not repeat Search. Query has no implicit Search state.";
    }
