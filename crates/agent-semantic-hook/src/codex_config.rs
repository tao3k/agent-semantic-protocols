...
pub fn codex_user_trust_state_status(
    ...
    let state = parsed
        ...
        .and_then(toml::Value::as_table);
    let (missing_events, stale_events) =
        codex_project_hook_trust_drift(state, &project_config_path, &project_root);
...
fn codex_project_hook_trust_drift(
    state: Option<&toml::map::Map<String, toml::Value>>,
    project_config_path: &Path,
    project_root: &Path,
) -> (Vec<String>, Vec<String>) {
    codex_hook_events().into_iter().fold(
        (Vec::new(), Vec::new()),
        |(mut missing_events, mut stale_events), event| {
