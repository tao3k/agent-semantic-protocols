...
fn owner_rank_entries(
    ...
    owner_ranks.values_mut().for_each(|rank| {
        ...
        rank.topology_query_axis_count = submodule_paths
            .iter()
            .find(|submodule_path| graph_path_is_under(&rank.path, submodule_path))
...
fn new_owner_rank(candidate: &GraphProjectionCandidate, first_index: usize) -> OwnerRank {
    OwnerRank {
...
fn update_owner_rank(
    ...
    if submodule_paths
        .iter()
        .any(|submodule_path| graph_path_is_under(&candidate.path, submodule_path))
    {
...
fn matched_query_axes(candidate: &GraphProjectionCandidate, query_axes: &[String]) -> Vec<String> {
...
fn owner_rank_evidence(candidate: &GraphProjectionCandidate) -> String {
...
fn is_parser_finder_local_candidate(candidate: &GraphProjectionCandidate) -> bool {
    matches!(
...
fn is_path_evidence_candidate(candidate: &GraphProjectionCandidate) -> bool {
    matches!(
        candidate.source.as_str(),
        "fd-query" | "finder-path" | "package-path-query" | "query-anchor"
    ) || matches!(
