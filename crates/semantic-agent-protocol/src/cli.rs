//! Process argument entrypoints for `semantic-agent-protocol`.

use crate::command::run_protocol_command;
use std::env;

/// Run the `semantic-agent-protocol` CLI using process arguments.
pub fn run_cli_from_env() -> Result<(), String> {
    run_protocol_command(env::args().skip(1).collect())
}

/// Run the `semantic-agent-protocol` CLI using caller-provided arguments.
