...
#[test]
fn repository_client_database_path_is_shared_across_workspaces() {
    ...
    let expected_project_client_dir = state_home
        .join("projects")
        .join("by-id")
        .join("project-123")
        .join("live")
    ...
    assert_eq!(workspace_a.project_client_dir, expected_project_client_dir);
    assert_eq!(
        workspace_a.project_client_db_path,
        expected_project_client_dir.join("facts.turso")
    );
    assert_eq!(
        workspace_a.project_client_db_path,
        workspace_b.project_client_db_path
    );
    assert_eq!(
        workspace_a.client_db_path,
        workspace_a.client_dir.join("facts.turso")
    );
