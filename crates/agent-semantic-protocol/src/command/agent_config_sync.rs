//! Owner-driven projection of ASP agent profiles into the host Codex home.
...
fn sync_agent_config() -> Result<(), String> {
    let project_catalog = std::env::current_dir()
    ...
    let agents_root = project_catalog
        .parent()
        .ok_or_else(|| format!("agent catalog has no parent: {}", project_catalog.display()))?;
    ...
    for route_key in loaded.registry.agents.keys() {
        for platform in loaded.registry.agents[route_key].platforms.keys() {
            let route = agent_semantic_config::agent_route_registry::compile_agent_route(
                &loaded, route_key, platform,
            )?;
            let source_path = agents_root.join(&route.projection);
            ...
            atomic_write(&state_agents.join(&route.projection), &source)?;
            if platform != "codex" {
                continue;
            }
            let model = route
            ...
            projections.insert(
                ...
                json!({
                    ...
                    "sessionName": route.session_name.as_str(),
