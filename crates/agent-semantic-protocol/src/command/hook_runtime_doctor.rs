...
use agent_semantic_hook::{
    DecisionKind, HOOK_PROTOCOL_ID, HookClassificationRequest, ReasonKind,
    RuntimeProviderHealthStatus, classify_hook_with_config, codex_global_hook_binary_path,
...
pub(super) fn run_doctor(args: &[String]) -> Result<(), String> {
    ...
    let root_hook = if client == "claude" {
        ...
        codex_global_hook_config_present(&config)
    };
    let plugin_hook = client == "codex"
        && super::hook_runtime_codex_plugin::codex_plugin_hook_present(&project_root)?;
    let hook_mode = hook_mode_label(client, root_hook, plugin_hook);
    let hook_binary_probe = crate::command::protocol_binary::protocol_binary_path_probe();
    let active_hook_binary_path = hook_binary_probe.path.clone();
    let active_contract_fingerprint = active_hook_binary_path
        .as_ref()
    ...
    let hook_binary = active_hook_binary_path.is_some();
    ...
    let project_trust_status = if client == "codex" {
        codex_user_trust_state_status(&project_config_path).ok()
    } else {
        None
    };
    let global_trust_status = if client == "codex" {
        codex_global_hook_trust_state_status(&config_path).ok()
    } else {
        None
    };
    ...
    let project_trust = project_trust_status
        .as_ref()
        .is_some_and(|status| status.project_trusted);
    let native_inline_hook_trust = global_trust_status
        .as_ref()
        .is_some_and(|status| status.trusted);
    let trust_mode = if plugin_hook && !root_hook {
        "plugin-host-managed"
    } else if root_hook {
        "native-inline-hook-injection"
    } else {
        "none"
    };
    let trust_status = if plugin_hook && !root_hook {
        "host-managed"
    } else if native_inline_hook_trust {
        "trusted"
    } else {
        "untrusted"
    };
    let trust_missing_count = global_trust_status
        .as_ref()
        .map(|status| status.missing_events.len())
        .unwrap_or(0);
    let trust_stale_count = global_trust_status
        .as_ref()
        .map(|status| status.stale_events.len())
        .unwrap_or(0);
    let trust_config = global_trust_status
        .as_ref()
    ...
    println!(
        ...
        trust_status,
        project_trust,
        native_inline_hook_trust,
        trust_missing_count,
        trust_stale_count,
        trust_config,
        configured_hook_binary_status,
        hook_binary,
        hook_binary_path,
...
fn hook_mode_label(client: &str, root_hook: bool, plugin_hook: bool) -> &'static str {
    if client != "codex" {
        return if root_hook {
            "client-config"
        } else {
    ...
    match (plugin_hook, root_hook) {
        (true, false) => "codex-plugin",
        (true, true) => "dual-authority-invalid",
        (false, true) => "codex-native-inline-debug",
        (false, false) => "missing",
    }
}
