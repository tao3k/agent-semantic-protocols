...
pub(crate) fn run_hook(root: &Path, state_home: &Path, activation: &Path, payload: Value) -> Value {
    crate::hook_testkit::run_hook(root, state_home, activation, payload)
}
...
fn run_hook_matrix_parallel(
    ...
    crate::hook_testkit::run_hook_matrix_parallel(root, state_home, activation, payloads)
}

fn warm_executable_without_hook_state(root: &Path, state_home: &Path) {
    crate::hook_testkit::warm_executable_without_hook_state(root, state_home)
}

pub(crate) fn write_fixture(root: &Path, state_home: &Path) {
    crate::hook_testkit::write_fixture(root, state_home)
}

fn assert_server_absent(state_home: &Path, stage: &str) {
    crate::hook_testkit::assert_server_absent(state_home, stage)
}

pub(crate) fn fixture_root() -> PathBuf {
    crate::hook_testkit::fixture_root()
}
