...
#[test]
fn zero_match_summary_is_single_and_explains_the_query_mode() {
    let summary = diagnostics::render_query_summary("rust", 0, 0);
    let guidance = diagnostics::render_query_miss_guidance("rust");
    let output = std::iter::once(summary)
        .chain(guidance)
        .collect::<Vec<_>>()
        .join("\n");

    assert_eq!(output.matches("[query-treesitter]").count(), 1);
    assert!(output.contains("status=no-matches"));
    assert!(output.contains("mode: structural Tree-sitter query"));
...
fn successful_summary_explains_compact_capture_rows() {
    let summary = diagnostics::render_query_summary("rust", 3, 2);
    ...
    assert!(diagnostics::render_query_match_guidance().contains("each `I=syntax:...` row"),);
}
