...
fn julia_language_facade_rewrites_compiled_provider_guide_commands() {
    ...
    assert!(
        stdout.contains(
            "|cmd lexical=asp julia search lexical --query <seed> --query <seed> owner tests --workspace . --view seeds"
        ),
        "{stdout}"
    );
    assert!(
