...
use crate::provider_command::support::{
    ...
    write_echo_provider, write_provider_bin_config,
};

#[test]
fn home_local_binary_overrides_runtime_profile_command_prefix() {
...
"#,
    ...
    let stdout = String::from_utf8(output.stdout).expect("stdout");
    assert!(
        stdout.contains("home-local args=[guide][languages/rust-lang-project-harness]"),
        "{stdout}"
    );
    assert!(!stdout.contains("profile args="), "{stdout}");
    assert!(
...
#[test]
fn home_local_binary_overrides_path_and_runtime_profile_for_nested_workspace() {
    let root = temp_project_root("provider-prefix-home-local-fixed");
    std::fs::remove_dir_all(root.join(".git")).expect("remove temp git marker");
    let cache_home = root.join(".cache-home");
    let home = root.join("home");
    let activation_path = cache_home.join("agent-semantic-protocol/hooks/activation.json");
    let bin_dir = root.join(".bin");
    write_echo_provider(&bin_dir, "provider-wrapper", "profile");
    write_echo_provider(&bin_dir, "rs-harness", "path-provider");

    let home_bin = home_local_bin(&root);
    write_echo_provider(&home_bin, "rs-harness", "home-local");
    write_activation_to(
        &root,
        &activation_path,
        &[provider(
    ...
    assert!(!stdout.contains("profile args="), "{stdout}");
    assert!(!stdout.contains("path-provider args="), "{stdout}");
    let _ = std::fs::remove_dir_all(root);
}
