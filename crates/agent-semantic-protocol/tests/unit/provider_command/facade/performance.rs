...
#[test]
fn search_lexical_broad_query_blocks_before_backend_collection() {
    let root = temp_project_root("search-lexical-broad-query-budget-gate");
    let bin_dir = root.join(".bin");
    let marker = root.join("provider-called");
    std::fs::create_dir_all(&bin_dir).expect("create bin dir");
    write_regular_search_fixtures(&root);
    ...
    let output = asp_command(&root)
        ...
        .output()
        .expect("run broad search lexical");
    ...
    let stdout = String::from_utf8(output.stdout).expect("stdout");
    assert!(
        stdout.contains("[search-lexical]")
            && stdout.contains("noOutput reason=query-too-broad")
            && stdout.contains(
                "nextCommand=asp fd -query 'gerbil-utils|list.ss|typed-combinator-style|r013'"
            ),
    ...
    assert_trace_elapsed_under_gate_ms(
        &["gerbil-scheme", "search", "lexical", "broad-query"],
        &stdout,
        ASP_BLOCKED_QUERY_PHASE_PERFORMANCE_GATE_MS,
    );
    let _ = std::fs::remove_dir_all(root);
