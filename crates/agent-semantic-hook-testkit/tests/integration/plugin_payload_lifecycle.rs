...
const SOURCE_LAUNCHER: &[u8] = include_bytes!("../../../../asp-codex-plugin/bin/asp-hook-exec");

#[test]
fn plugin_preserves_temporary_subagent_events_as_observational_extension_points() {
    let hooks: serde_json::Value = serde_json::from_slice(SOURCE_HOOKS).expect("parse hooks");
    ...
    for (event, command) in [
        ("SubagentStart", "subagent-start --client codex"),
        ("SubagentStop", "subagent-stop --client codex"),
    ] {
        let groups = events[event].as_array().expect("lifecycle groups");
        let handler = groups[0]["hooks"][0]["command"]
            .as_str()
            .expect("lifecycle command");
        assert!(handler.contains(command), "event={event} handler={handler}");
    }
    assert!(
        SOURCE_HOOKS
            .windows(b"asp session --agents choice-plane".len())
            .all(|window| window != b"asp session --agents choice-plane")
    );
}
...
fn payload_validation_rejects_runtime_coupled_launcher() {
    ...
    assert!(
        load_codex_plugin_payload_identity(&source)
            .expect_err("Client-coupled launcher must fail")
