...
fn verified_explorer_search_is_authorized_once_and_post_tool_remains_observational() {
    ...
    for payload in &payloads {
        let pre_tool =
            classify_codex_plugin_scenario(&runtime, &config, "pre-tool", payload, "Bash")
                .expect("classify registered Explorer search authorization");
        assert_eq!(pre_tool["decision"], "allow", "decision={pre_tool:#}");
        assert_eq!(
            pre_tool["fields"]["dispatchAdmission"], "config-agent-role",
            "decision={pre_tool:#}"
        );
...
#[test]
fn config_agent_roles_satisfy_only_their_declared_dispatch_routes() {
    let cases = [
        (
            "asp_explorer",
            "asp typescript search playbook projectRoot --workspace .",
    ...
    for (agent_name, command, expected_decision, intent) in cases {
        ...
        if expected_decision == "deny" {
            assert_eq!(decision["reasonKind"], "agent-choice-required");
        } else {
