...
pub(crate) fn record_active_context(
    ...
    let Some(project_root) = infer_project_root(activation_path) else {
        return;
    };
    let project_root_hash = stable_hash_hex(&project_root.display().to_string());
    let Some(log_root) = resolve_log_root(&project_root, &project_root_hash) else {
        return;
    };
    ...
    .or_else(|| json_string(payload, &["hookRunId"]))
    .or_else(|| json_string(payload, &["hook_run_id"]))
    .or_else(|| json_string(payload, &["run_id"]));
    let parent_event_id = env_first(&["SEMANTIC_PROTOCOL_PARENT_EVENT_ID"])
        .or_else(|| hook_run_id.clone())
        .unwrap_or_else(|| make_hook_event_id(event));
    ...
    .or_else(|| json_string(payload, &["conversation_id"]))
    .or_else(|| {
        hook_run_id
            .as_ref()
    ...
    let decision_kind = serde_json::to_value(decision)
        .ok()
