...
const CODEX_TOOL_MATCHER: &str =
    "Read|read_file|mcp__.*__read_file|functions.exec_command|multi_tool_use.parallel|Bash|Shell";
...
fn codex_hook_command(hook_event: &str) -> String {
    format!(
        "repo_root=\"$(git rev-parse --show-toplevel 2>/dev/null || pwd)\"\ncd \"$repo_root\"\nactivation=\"$repo_root/.codex/semantic-agent-hook/activation.json\"\nexec semantic-agent-hook hook --client codex {hook_event} --activation \"$activation\"\n"
    )
}
