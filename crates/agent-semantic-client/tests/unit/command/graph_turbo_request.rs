use super::decode_graph_turbo_request;
...
fn search_packet_remains_an_identity_free_rank_intent_until_runtime_admission() {
    ...
    let message = decode_graph_turbo_request(&encoded).expect("decode rank intent");
...
fn rank_intent_rejects_a_non_object_packet() {
    ...
    let encoded = serde_json::to_vec(&packet).expect("encode fixture");
    let error =
        decode_graph_turbo_request(&encoded).expect_err("non-object rank intent is rejected");
