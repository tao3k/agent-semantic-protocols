...
#[cfg(test)]
pub(crate) async fn prepare_runtime_server_owner_projection_with_registry_async(
...
#[cfg(test)]
pub(crate) async fn prepare_runtime_server_workspace_generation_with_registry_async(
    project_root: PathBuf,
    snapshot: ProviderRegistrySnapshot,
    ...
    let registry = snapshot.evidence(&project_root);
    let collection = super::collect::collect_source_index_scope_async(
        &project_root,
        &snapshot,
        &collection_scope,
    )
