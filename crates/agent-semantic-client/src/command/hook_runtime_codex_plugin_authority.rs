use super::{ASP_CODEX_PLUGIN_NAME, CodexHookConfigInstallReceipt, write_codex_config_atomically};
...
pub(in crate::command) fn validate_codex_plugin_source_payload() -> Result<String, String> {
    ...
    {
        return Err(
            "ASP Codex plugin Hook launcher must exec only the immutable HookGeneration evaluator"
                .to_string(),
        );
    }
