...
pub(crate) async fn render_search_packet_bytes(packet_bytes: Bytes) -> Option<Bytes> {
...
pub(crate) async fn render_search_packet_bytes_with_receipt(
    packet_bytes: Bytes,
...
pub(crate) async fn render_search_packet_artifact_stdout(artifact_path: &Path) -> Option<Bytes> {
    let metadata = fs::metadata(artifact_path).ok()?;
    if !metadata.is_file() || metadata.len() > MAX_CACHE_REPLAY_ARTIFACT_BYTES {
        return None;
    }
    run_graph_render_packet(artifact_path, MAX_CACHE_REPLAY_ARTIFACT_BYTES).await
}
