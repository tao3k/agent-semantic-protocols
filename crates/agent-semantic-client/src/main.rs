...
fn main() -> std::process::ExitCode {
    ...
    let hook_dispatch =
        agent_semantic_client::hook_bootstrap::is_hook_event_dispatch(hook_args.clone());
    if hook_dispatch && std::env::var_os("ASP_HOOK_BOOTSTRAP_TRACE").is_some() {
        eprintln!("[asp-hook] route=process-entry-hook-dispatch");
    }
    if let Some(result) =
        agent_semantic_client::hook_bootstrap::process_entry_no_agent_bypass(&hook_args)
    {
    ...
    if synchronous_hook_dispatch {
        ...
        let code = agent_semantic_client::hook_bootstrap::run_synchronous_hook_bootstrap_from_env();
        agent_semantic_client::hook_bootstrap::terminate_hook_process(code)
    }
    if hook_dispatch && std::env::var_os("ASP_HOOK_BOOTSTRAP_TRACE").is_some() {
    ...
    let no_agent_client = !hook_dispatch
        && !daemon
    ...
    let outcome = runtime.block_on(async {
        if hook_dispatch {
            ProcessOutcome::Hook(
                agent_semantic_client::hook_bootstrap::run_hook_bootstrap_from_env().await,
            )
        } else if no_agent_client {
            ProcessOutcome::Command(agent_semantic_client::run_binary_from_env().await)
        } else {
