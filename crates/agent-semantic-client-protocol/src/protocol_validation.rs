...
//! Validation and lifecycle execution for ASP Client Protocol values.
...
use super::ClientAdmissionError;
use super::ClientConformanceReceipt;
use super::ClientConformanceSuite;
use super::ClientFrame;
use super::ClientFrameBase;
use super::ClientMethod;
use super::ClientOutcome;
use super::ClientParameterCardinality;
use super::ClientParameterSource;
use super::ClientParameterType;
use super::ClientProtocolCatalog;
use super::ClientSession;
use super::ClientSessionState;
...
impl ClientProtocolCatalog {
    pub fn validate(&self) -> Result<(), ClientAdmissionError> {
        ...
        for method in &self.methods {
            ...
            validate_dotted_id("routeId", method.route_id.as_str())?;
            for (field, value) in [
                ("requestSchemaId", method.request_schema_id.as_str()),
                ("responseSchemaId", method.response_schema_id.as_str()),
            ] {
                if value.is_empty() {
                    return Err(error(
...
/// Executes every conformance case against the reference session admission state machine.
        ...
        let (outcome, reason_kind) = match session.admit(&case.request, catalog) {
            ...
            Err(error) => {
                ...
                (
                    outcome,
                    Some(
                        crate::ClientReasonKind::new(error.reason_kind)
                            .expect("static client reason kind"),
                    ),
                )
            }
        };
        if outcome != case.expected_outcome
            || case
                ...
                .is_some_and(|expected| {
                    reason_kind.as_ref().map(crate::ClientReasonKind::as_str) != Some(expected)
                })
        {
            return Err(error(
