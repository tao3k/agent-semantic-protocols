...
fn missing_home_local_binary_reports_install_language_without_runtime_profile_fallback() {
    ...
    assert!(
        error.contains("$ASP_STATE_HOME/runtime/bin/rs-harness")
            || error.contains(".agent-semantic-protocols/runtime/bin/rs-harness")
    );
...
fn install_home_provider(name: &str, binary: &str, body: &str) -> HomeProviderFixture {
    ...
    let provider_path = std::fs::canonicalize(provider_path).expect("canonical provider path");
    HomeProviderFixture {
        _home: home,
        provider_path,
    }
