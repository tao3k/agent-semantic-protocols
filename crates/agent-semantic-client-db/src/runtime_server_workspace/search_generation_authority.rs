...
impl WorkspaceSearchGenerationAuthority {
    pub(super) fn from_generation_with_projection_digests(
        ...
        let execution_binding = generation
            .runtime_provider_execution_binding
            .as_ref()
            .ok_or_else(|| {
                "Runtime Server search authority requires a provider execution binding".to_owned()
            })?;
        execution_binding.validate()?;
        if execution_binding.workspace_id != generation.workspace_identity {
            return Err(
                "Runtime Server search authority workspaceId differs from its generation"
                    .to_owned(),
            );
        }
        Ok(Self {
            ...
            project_id: execution_binding.project_id.clone(),
            workspace_id: execution_binding.workspace_id.clone(),
            ...
            runtime_provider_execution_binding: generation
                .runtime_provider_execution_binding
                .clone(),
            content_search_generation: generation.content_search_generation.clone(),
    ...
    pub fn validate_binding(&self, project_id: &str, workspace_id: &str) -> Result<(), String> {
        ...
        {
            return Err(format!(
                "Runtime Server search generation authority binding mismatch: expectedProjectId={} actualProjectId={} expectedWorkspaceId={} actualWorkspaceId={}",
                project_id, self.project_id, workspace_id, self.workspace_id,
            ));
        }
        self.content_search_generation.validate()?;
        let binding = self
            .runtime_provider_execution_binding
            .as_ref()
            .ok_or_else(|| {
                "Runtime Server search authority lacks provider execution binding; canonical generation rebuild required"
                    .to_owned()
            })?;
        binding.validate()?;
        if binding.project_id != self.project_id
            || binding.workspace_id != self.workspace_id
            || binding.source_snapshot_digest != self.source_snapshot.root_integrity_reference()?
        {
            return Err(
                "Runtime Server search authority provider execution binding drift".to_owned(),
            );
        }
        let content_identity = self.content_search_generation.identity();
        if content_identity.project_id != self.project_id
            || content_identity.workspace_id != self.workspace_id
            || content_identity.source_root_digest
                != agent_semantic_search::canonical_blake3_digest(
                    &self.source_snapshot.root_digest,
                )?
        ...
        {
            return Err("Runtime Server content search generation authority drift".to_owned());
        }
...
pub(crate) async fn read_search_generation_authority_segment(
    ...
    authority.validate_binding(project_id, workspace_id)?;
...
fn publish_resident_generation(
    ...
    authority.validate_binding(&authority.project_id, &authority.workspace_id)?;
    if let Some(current) = committed.as_ref() {
        let current_authority = current.authority();
        if authority.project_id != current_authority.project_id
            || authority.workspace_id != current_authority.workspace_id
        {
...
pub(crate) async fn publish_search_generation_authority_segment(
    ...
    let path =
        search_generation_authority_segment_path(generation_pointer_path, authority.active_epoch)?;
