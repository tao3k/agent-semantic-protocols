use super::types::ClientDbStructuralQueryKey;

pub(super) struct StructuralSearchProjection {
    pub(super) query_keys_json: String,
    pub(super) search_text: String,
}

pub(super) fn structural_search_projection<'a>(
    fixed_terms: impl IntoIterator<Item = &'a str>,
    query_keys: &'a [ClientDbStructuralQueryKey],
) -> Result<StructuralSearchProjection, String> {
    let mut search_text = String::new();
    let query_key_values = structural_search_text(&mut search_text, fixed_terms, query_keys);
    let query_keys_json = serde_json::to_string(&query_key_values)
        .map_err(|error| format!("failed to serialize structural query keys: {error}"))?;
    Ok(StructuralSearchProjection {
        query_keys_json,
        search_text,
    })
}
...
pub(super) fn parse_query_keys(
    json: &str,
    column: usize,
) -> rusqlite::Result<Vec<ClientDbStructuralQueryKey>> {
...
pub(super) fn structural_like_query(query: &str) -> String {
    let escaped = query
        .to_ascii_lowercase()
        .replace('\\', "\\\\")
        .replace('%', "\\%")
