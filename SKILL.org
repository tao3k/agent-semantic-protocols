...
=fork_turns="none"= and include a self-contained ASP Explorer branch prompt
plus the safe ASP command group in the message.

Keep model, reasoning, and agent-type defaults in Codex config, not in the
prompt. Spawn one read-only instance per independent =actionFrontier= or
=targetActions= branch, and require a compact =[asp-search-subagent]= receipt
with =role=, =action=, =evidence=, =missing=, =next=, and =risk=.
