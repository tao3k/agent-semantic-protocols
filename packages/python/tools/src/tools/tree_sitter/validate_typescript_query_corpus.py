...
def validate_profile(profile: dict[str, object]) -> None:
    ...
    assert_asp_tree_sitter_contract(
        profile,
        extra_paths=("packages/python/tools/src/tools/tree_sitter/validate_typescript_query_corpus.py",),
    )
    assert (
...
def parse_corpus(path: Path) -> list[CorpusCase]:
    ...
    while index < len(lines):
        ...
        if index >= len(lines):
            break
        case, index = _parse_case_at(path, lines, index)
        cases.append(case)
    return cases
...
def _parse_case_at(
    ...
    metadata, index = _parse_metadata(path, title, lines, index + 3)
    source, index = _collect_until(lines, index + 1, EXPECTED_SEPARATOR)
    assert index < len(lines), f"{path}:{title}: missing expected separator"
    expected, index = _collect_until(lines, index + 1, SEPARATOR)
    return corpus_case(title, metadata, source, expected), index
