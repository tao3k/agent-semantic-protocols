...
def validate_step(
    ...
    _validate_command_flow_expectation(expect, result)
...
def _validate_command_flow_expectation(expect: dict[str, Any], result: StepResult) -> None:
    flow_expect = dict_value(expect.get("commandFlow"))
    if not flow_expect:
        return
    command_flow = dict_value(result.observations.get("commandFlow"))
    token_cost = dict_value(result.observations.get("tokenCost"))
    if not command_flow:
        result.errors.append("commandFlow missing from agent observations")
        return

    _validate_command_flow_max(flow_expect, command_flow, result)
    _validate_command_flow_min(flow_expect, command_flow, result)
    _validate_frontier_context_metrics(flow_expect, command_flow, result)
    if bool(flow_expect.get("requireTokenCost")) and not token_cost:
        result.errors.append("tokenCost missing from agent observations")
    if bool(flow_expect.get("requireReadLoopMemory")):
        _validate_read_loop_memory(command_flow, result)
    for stage in string_list(flow_expect.get("requiredStages")):
        if not _command_flow_stage_present(stage, command_flow):
            result.errors.append(f"commandFlow missing required stage {stage!r}")
    for stage in string_list(flow_expect.get("forbiddenStages")):
        if _command_flow_stage_present(stage, command_flow):
            result.errors.append(f"commandFlow contains forbidden stage {stage!r}")
...
def _validate_command_flow_max(
    flow_expect: dict[str, Any],
    command_flow: dict[str, Any],
    result: StepResult,
) -> None:
    fields = {
        ...
        "maxSearchPlaybookCommands": "searchPlaybookCommands",
    ...
    for expect_key, flow_key in fields.items():
        maximum = optional_int(flow_expect.get(expect_key))
        observed = optional_int(command_flow.get(flow_key))
        if (
            maximum is not None
            and observed is None
            and flow_key == "aspCommandOutputBytes"
        ):
            result.errors.append(f"commandFlow {flow_key} missing for {expect_key}")
            continue
        value = observed or 0
        if maximum is not None and value > maximum:
            result.errors.append(
                f"commandFlow {flow_key}={value} exceeds {expect_key}={maximum}"
            )


def _validate_command_flow_min(
    flow_expect: dict[str, Any],
    command_flow: dict[str, Any],
    result: StepResult,
) -> None:
    minimum = optional_int(flow_expect.get("minQuerySelectorCommands"))
    value = optional_int(command_flow.get("querySelectorCommands")) or 0
    if minimum is not None and value < minimum:
        result.errors.append(
            f"commandFlow querySelectorCommands={value} below minQuerySelectorCommands={minimum}"
        )
...
def _validate_frontier_context_metrics(
    ...
    for expect_key, flow_key in fields.items():
        minimum = optional_float(flow_expect.get(expect_key))
        if minimum is None:
            continue
        observed = optional_float(command_flow.get(flow_key))
        if observed is None:
            result.errors.append(f"commandFlow {flow_key} missing for {expect_key}")
            continue
        if observed < minimum:
            result.errors.append(
                f"commandFlow {flow_key}={observed:.4f} below {expect_key}={minimum:.4f}"
            )


def _validate_read_loop_memory(command_flow: dict[str, Any], result: StepResult) -> None:
    memory = dict_value(command_flow.get("readLoopMemory"))
    if not memory:
        result.errors.append("commandFlow readLoopMemory missing")
        return
    entries = memory.get("entries", [])
    if not isinstance(entries, list) or not entries:
        result.errors.append("commandFlow readLoopMemory entries missing")
    entry_count = optional_int(memory.get("entryCount")) or 0
    if entry_count < 1:
        result.errors.append("commandFlow readLoopMemory entryCount below 1")


def _command_flow_stage_present(stage: str, command_flow: dict[str, Any]) -> bool:
    if stage == "search-playbook":
        return (optional_int(command_flow.get("searchPlaybookCommands")) or 0) > 0
    if stage == "query-selector":
        return (optional_int(command_flow.get("querySelectorCommands")) or 0) > 0
    if stage == "treesitter-query":
        return (optional_int(command_flow.get("treesitterQueryCommands")) or 0) > 0
    if stage == "direct-read":
        return (optional_int(command_flow.get("directReadCommands")) or 0) > 0
    if stage == "repeated-commands":
        return (optional_int(command_flow.get("repeatedCommands")) or 0) > 0
    if stage == "read-loop-risk":
        duplicate_selectors = (
            optional_int(command_flow.get("readLoopDuplicateSelectors")) or 0
        )
        adjacent_windows = (
            optional_int(command_flow.get("readLoopAdjacentRangeWindows")) or 0
        )
        same_owner_scans = optional_int(command_flow.get("readLoopSameOwnerScans")) or 0
        return duplicate_selectors + adjacent_windows + same_owner_scans > 0
    if stage == "read-loop-memory-risk":
        return (optional_int(command_flow.get("readLoopMemorySuppressibleReads")) or 0) > 0
    return False
