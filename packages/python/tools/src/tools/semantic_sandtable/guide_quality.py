...
def validate_guide_quality(
    ...
    if not _requires_decision_validation(guide):
        return
...
def _graph_aliases(stdout: str) -> dict[str, str]:
    ...
    for line in stdout.splitlines():
        if line.startswith("aliases="):
            ...
            continue
        if line.startswith("aliases: graph:{") and line.endswith("}"):
            body = line.removeprefix("aliases: graph:{").removesuffix("}")
            for entry in body.split(","):
                if "=" not in entry:
                    continue
                alias, node_kind = entry.split("=", 1)
                aliases[alias.strip()] = node_kind.strip().split(":", 1)[0]
    return aliases


def _requires_decision_validation(guide: dict[str, Any]) -> bool:
    return any(
        key in guide
        for key in (
            "reasonKind",
