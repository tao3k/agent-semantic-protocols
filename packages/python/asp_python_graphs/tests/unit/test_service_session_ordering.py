...
def test_control_before_data_cancellation_suppresses_late_evaluate(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    session = opened_session(initial_sequence=1)
    session.register_request("eval-1")
    cancelled = session.handle(
        message("cancel", "cancel-1", 2, cancellationId="eval-1")
    )
    assert cancelled["payload"]["state"] == "cancelled"  # type: ignore[index]

    def reject_algorithm_commit(*_args: object, **_kwargs: object) -> None:
        pytest.fail("cancelled evaluate reached the algorithm commit path")

    monkeypatch.setattr(session, "_evaluate", reject_algorithm_commit)
    late = session.handle(
        message(
            "evaluate",
            "eval-1",
    ...
    assert late["payload"]["state"] == "cancelled"  # type: ignore[index]
    assert "result" not in late["payload"]  # type: ignore[operator]
