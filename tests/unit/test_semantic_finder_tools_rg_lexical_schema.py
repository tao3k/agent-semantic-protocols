"""Validate rg+lexical finder pipeline diversity."""
...
def _rg_lexical_path_pipeline() -> dict:
    return {
        ...
        "stages": [
            ...
            _pipeline_stage("lexical-path-rank", "lexical", "fuzzy-filter", "fuzzy", "path-list", "path-list"),
        ],
        "output": {
...
def _rg_lexical_provenance_samples() -> list[dict]:
    return [
        _rg_lexical_provenance(
            "lexical-rg-lines",
            "source-lines",
            "run codex hook",
            120,
            ...
            [
                ...
                _provenance_stage("lexical-rank", "lexical", "fuzzy-filter", "rg-n", "line-list", 120, 12),
            ],
        ),
        _rg_lexical_provenance(
            "lexical-rg-paths",
            ...
            [
                ...
                _provenance_stage("lexical-path-rank", "lexical", "fuzzy-filter", "path-list", "path-list", 48, 6),
            ],
        ),
        _rg_lexical_provenance(
            "lexical-rg-owner-labels",
            "owner-labels",
            "protocol owner",
            64,
            8,
...
def _rg_lexical_provenance(
    ...
    return {
        ...
        "fuzzyFilter": {"toolId": "lexical", "query": query, "scoreBasis": "rank"},
        "stages": stages,
    }


class SemanticFinderToolsRgLexicalSchemaTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_catalog_supports_candidate_basis_variants(self) -> None:
        ...
        document["pipelines"].extend(
            [_rg_lexical_path_pipeline(), _rg_lexical_owner_label_pipeline()]
        )
    ...
    def test_provenance_supports_candidate_basis_variants(self) -> None:
        document = copy.deepcopy(_valid_catalog())
        document["provenanceSamples"] = _rg_lexical_provenance_samples()
        self._assert_valid(document)
        self.assertEqual(
            ["source-lines", "paths", "owner-labels"],
