...
from tests.unit._asp_python_query_search_perf_support import (
    SEARCH_TERM,
...
def test_asp_python_registered_search_views_are_fast_on_warm_path(
    tmp_path: Path,
) -> None:
    project = write_python_fixture(tmp_path)
    descriptors = [
    ...
    assert "search/playbook" in registered_methods
    assert "search/playbook" not in registered_methods
